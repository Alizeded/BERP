...
sim_rir_manifest = pd.read_csv("./data/noiseReverbSpeech/train_manifest_RIR_sim.csv")

# choose the same length of speech and rir
speech_long_manifest = speech_long_manifest.sample(
    n=len(sim_rir_manifest), random_state=2036
)
...
j = 0
reverbSpeech = []
for i in tqdm(range(len(sim_rir_manifest))):
    results = rir_synthesis_noise(
        sim_rir_manifest.iloc[i], speech_long_manifest.iloc[i], noise_database
    )
...
# reshuffle the data
reverbSpeech = reverbSpeech.sample(frac=1, random_state=2036)

reverbSpeech.to_csv(
    os.path.join(save_path_label, "train_sim_manifest.csv"), index=False
)
