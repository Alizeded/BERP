...
class ReverbSpeechDataModuleJointEstE2E(LightningDataModule):
    ...
    def __init__(
        ...
        (  # load data
            ...
            os.path.join(data_dir, path_test),
        )
        ...
        self.tr_train = self.labels_train["tr"]
        self.tr_val = self.labels_val["tr"]
        self.tr_test = self.labels_test["tr"]
    ...
    def collate_fn(self, batch):
        ...
        trs = [sample["t60"] for sample in batch]
    ...
    def _get_item_and_labels(self, data_dir: str, path_raw: str, path_test: str):
        if (
            os.path.isfile(os.path.join(data_dir, "train_sim_manifest_alt.csv"))
            and os.path.isfile(os.path.join(data_dir, "test_manifest_alt.csv"))
            and os.path.isfile(os.path.join(data_dir, "val_sim_manifest_alt.csv"))
        ):
            manifest_train_df = pd.read_csv(
                os.path.join(data_dir, "train_sim_manifest_alt.csv")
            )
            manifest_val_df = pd.read_csv(
                os.path.join(data_dir, "val_sim_manifest_alt.csv")
            )
            ...
            tr_train = manifest_train_df["T60"].tolist()
            tr_val = manifest_val_df["T60"].tolist()
            tr_test = manifest_test_df["T60"].tolist()
            ...
            d50_train = manifest_train_df["D50"] / 100
            d50_train = d50_train.tolist()
            d50_val = manifest_val_df["D50"] / 100
            d50_val = d50_val.tolist()
            d50_test = manifest_test_df["D50"].tolist()

            ts_train = manifest_train_df["TS"].tolist()
            ts_val = manifest_val_df["TS"].tolist()
