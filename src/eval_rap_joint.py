...
def rap_eval_segment(
    ...
    joint_pred_segment: Dict[str, Any],
    ...
    for n in range(segment_size):
        Ti_hat = joint_pred_segment["Th_hat"][n]
        Td_hat = joint_pred_segment["Tt_hat"][n]
        volume_log_hat = joint_pred_segment["volume_hat"][n]
...
def rap_eval(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    log.info(
        f"Loading Ti, Td, and volume joint prediction from <{cfg.joint_pred_path}>"
    )
    joint_pred = torch.load(cfg.joint_pred_path)
    ...
    if cfg.multithreaded:
        with ThreadPoolExecutor(max_workers=12) as executor:
            ...
            for i, rir_segment in enumerate(rir_segments):
                joint_pred_segment = joint_pred[i]
        ...
        with Progress(
            ...
            for i, rir_segment in enumerate(rir_segments):
                ...
                progress.update(task, advance=1)
                joint_pred_segment = joint_pred[i]
                rap_paired = rap_eval_segment(cfg, rir_segment, joint_pred_segment)
                rap_pairs.append(rap_paired)
...
def main(cfg: DictConfig) -> None:
    ...
    rap_eval_results = agg_pair(rap_eval_results, batch_size=cfg.batch_size)
    ...
    log.info(f"Saving RAP evaluation metric to <{cfg.rap_eval_metric_path}>")
