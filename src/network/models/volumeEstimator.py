...
class VolumeEstimator(nn.Module):
    ...
    def __init__(
        ...
        if self.decoder_type == "parametric_predictor":
            # parametric predictor for volume
    ...
    def parametric_predictor_forward(self, x: torch.Tensor):
        ...
        volume_hat = self.parametric_predictor(x)

        volume_hat = volume_hat.squeeze()  # B x T x 1 -> B x T
    ...
    def forward(
        ...
        # transformer decoder
        if self.decoder_type == "parametric_predictor":
            volume_hat = self.parametric_predictor_forward(x)
        else:
            raise NotImplementedError("Only parametric predictor is supported")
