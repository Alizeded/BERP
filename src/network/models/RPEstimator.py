...
class RPEstimator(nn.Module):
    ...
    def parametric_predictor_forward(self, x: torch.Tensor):
        ...
        param_hat = self.parametric_predictor(x)

        param_hat = param_hat.squeeze()  # B x T x 1 -> B x T
    ...
    def forward(
        ...
        """
        Forward pass of volume module.

        Parameters:
            x (torch.Tensor): Input tensor of shape (batch_size, freq_bins, seq_len) or (batch_size, seq_len).
            padding_mask (torch.Tensor, optional): Padding mask tensor of shape (batch_size, seq_len). Defaults to None.

        Returns:
            volume_hat (torch.Tensor): Estimated volume of shape (batch_size, seq_len).
            padding_mask (torch.Tensor): Padding mask tensor of shape (batch_size, seq_len).
        """
        ...
        # transformer decoder
        if self.decoder_type == "parametric_predictor":
            param_hat = self.parametric_predictor_forward(x)
        else:
            raise NotImplementedError("Only parametric predictor is supported")

        return {f"{self.module}_hat": param_hat, "padding_mask": padding_mask}
