...
class JointEstimator(nn.Module):
    ...
    def __init__(
        ...
        if self.decoder_type == "parametric_predictor":
            ...
            # parameteric predictor for alcons
            self.parametric_predictor_alcons = ParametricPredictor(
                in_dim=embed_dim,
                out_dim=ch_out,
                num_layers=num_layers_decoder,
                num_channels=num_channels_decoder,
            ...
            # parameteric predictor for edt
            self.parametric_predictor_edt = ParametricPredictor(
                in_dim=embed_dim,
                out_dim=ch_out,
                num_layers=num_layers_decoder,
                num_channels=num_channels_decoder,
            ...
            # parameteric predictor for d50
            self.parametric_predictor_d50 = ParametricPredictor(
                in_dim=embed_dim,
                out_dim=ch_out,
                num_layers=num_layers_decoder,
                num_channels=num_channels_decoder,
            ...
            # parameteric predictor for ts
            self.parametric_predictor_ts = ParametricPredictor(
                in_dim=embed_dim,
                out_dim=ch_out,
                num_layers=num_layers_decoder,
                num_channels=num_channels_decoder,
    ...
    def parametric_predictor_forward(self, x: torch.Tensor) -> tuple[torch.Tensor]:
        ...
        alcons_hat = self.parametric_predictor_alcons(x)
        tr_hat = self.parametric_predictor_tr(x)
        edt_hat = self.parametric_predictor_edt(x)
        c80_hat = self.parametric_predictor_c80(x)
        c50_hat = self.parametric_predictor_c50(x)
        d50_hat = self.parametric_predictor_d50(x)
        ts_hat = self.parametric_predictor_ts(x)
