...
class rirRegressorModuleBaselineREnet(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
        ...
        # loss function
        self.loss_fn_Th = MSELoss()
        self.loss_fn_Tt = MSELoss()
    ...
    def model_step(
        ...
        loss_Th = self.loss_fn_Th(Th_hat, Th)
        loss_Tt = self.loss_fn_Tt(Tt_hat, Tt)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        total_loss_best = self.val_loss_best_Th + self.val_loss_best_Tt
    ...
    def test_step(
        ...
        Th_hat = Th_hat[:, -1]  # only the last time step
        Tt_hat = Tt_hat[:, -1]  # only the last time step
...
if __name__ == "__main__":
    ...
    _ = rirRegressorModuleBaselineREnet(cfg.model.rirRegressorModule)
