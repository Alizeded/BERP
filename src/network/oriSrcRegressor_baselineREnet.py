...
class OriSrcRegressorModuleBaselineREnet(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
    ...
    def model_step(
        ...
        ori_azimuth = repeat(ori_azimuth, "b -> b t", t=ori_azimuth_hat.shape[1])
        ori_elevation = repeat(ori_elevation, "b -> b t", t=ori_elevation_hat.shape[1])
    ...
    def test_step(
        ...
        ori_azimuth_hat = ori_azimuth_hat[:, -1]  # take the last time step
        ori_elevation_hat = ori_elevation_hat[:, -1]  # take the last time step
    ...
    def predict_step(
        ...
        ori_azimuth_hat = ori_azimuth_hat[:, -1]  # take the last time step
        ori_elevation_hat = ori_elevation_hat[:, -1]
...
if __name__ == "__main__":
    ...
    cfg = omegaconf.OmegaConf.load(
        root / "configs" / "model" / "network_baselineREnet.yaml"
    )
    _ = hydra.utils.instantiate(cfg.model.oriSrcRegressorModule)

    _ = OriSrcRegressorModuleBaselineREnet(cfg.model.oriSrcRegressorModule)
