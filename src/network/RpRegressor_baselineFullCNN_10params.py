...
class JointRegressorModuleBaselineFullCNN(LightningModule):
    ...
    def model_step(
        ...
        loss_alcons = self.joint_loss(alcons_hat, alcons)
        loss_t60 = self.joint_loss(t60_hat, t60)
        loss_edt = self.joint_loss(edt_hat, edt)
        loss_c80 = self.joint_loss(c80_hat, c80)
        loss_c50 = self.joint_loss(c50_hat, c50)
        loss_d50 = self.joint_loss(d50_hat, d50)
        loss_ts = self.joint_loss(ts_hat, ts)
    ...
    def test_step(
        ...
        corrcoef_dist_src = self.joint_corrcoef_test(dist_src_hat, dist_src).abs()
