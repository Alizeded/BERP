...
class JointRegressorModuleBaselineREnet(LightningModule):
    ...
    def model_step(
        ...
        Th = repeat(Th, "b -> b t", t=Th_hat.shape[1])
        Tt = repeat(Tt, "b -> b t", t=Tt_hat.shape[1])
        volume = repeat(volume, "b -> b t", t=volume_hat.shape[1])
        dist_src = repeat(dist_src, "b -> b t", t=dist_src_hat.shape[1])
        azimuth_src = repeat(azimuth_src, "b -> b t", t=azimuth_src_hat.shape[1])
        elevation_src = repeat(elevation_src, "b -> b t", t=elevation_src_hat.shape[1])

        loss_Th = self.joint_loss(Th_hat, Th)
        loss_Tt = self.joint_loss(Tt_hat, Tt)
        loss_volume = self.joint_loss(volume_hat, volume)
        loss_dist_src = self.joint_loss(dist_src_hat, dist_src)
        loss_azimuth_src = self.joint_loss(azimuth_src_hat, azimuth_src)
        loss_elevation_src = self.joint_loss(elevation_src_hat, elevation_src)
    ...
    def test_step(
        ...
        Th_hat = Th_hat[:, -1]  # take the last time step
        Tt_hat = Tt_hat[:, -1]  # take the last time step
        volume_hat = volume_hat[:, -1]  # take the last time step
        dist_src_hat = dist_src_hat[:, -1]  # take the last time step
        azimuth_src_hat = azimuth_src_hat[:, -1]  # take the last time step
        elevation_src_hat = elevation_src_hat[:, -1]  # take the last time step
